...
check export_fixture RedCube References/Blender/RedCube.blend
check export_fixture CubeWith4Animations References/Blender/CubeWith4Animations.blend \
  export-animation=true author-animation-library=true
check export_fixture SkinnedLimb References/Blender/t12_skinned_limb.blend \
  export-animation=true author-animation-library=true

mkdir -p "$OUT/exports/RedCube"
check python3 Plugin --blender "$BLENDERTORCP_BLENDER" --timeout 900 \
  export References/Blender/RedCube.blend \
  -o "$OUT/exports/RedCube/RedCube.usdz" --format USDZ --diagnostics
...
# this proves.
mkdir -p "$OUT/exports/SpecularTint"
set +e
python3 Plugin --blender "$BLENDERTORCP_BLENDER" --json --timeout 900 \
  export References/Blender/t21_specular_tint_refusal.blend \
  -o "$OUT/exports/SpecularTint/SpecularTint.usdc" --format USDC --diagnostics \
  > "$OUT/exports/SpecularTint/expected-portable-rejection.json"
tint_status=$?
set -e
if [[ "$tint_status" -eq 0 ]]; then
  ...
  TINT_REPORT="$OUT/exports/SpecularTint/expected-portable-rejection.json" python3 - <<'PY'
import json, os, sys
...
PY
  ...
  check test ! -e "$OUT/exports/SpecularTint/SpecularTint.usdc"
fi
...
# ---------------------------------------------------------------------------
for asset in \
  "$OUT/exports/RedCube/RedCube.usdc" \
  "$OUT/exports/CubeWith4Animations/CubeWith4Animations.usdc" \
  "$OUT/exports/SkinnedLimb/SkinnedLimb.usdc" \
  "$OUT/exports/RedCube/RedCube.usdz"
do
  [ -e "$asset" ] || continue
...
# ---------------------------------------------------------------------------
if [ -e "$OUT/exports/RedCube/RedCube.usdc" ]; then
  profile="$OUT/RedCube-material-profile.usda"
  xcrun usdcat "$OUT/exports/RedCube/RedCube.usdc" --out "$profile"
...
for platform in xros xrsimulator macosx iphoneos iphonesimulator appletvos appletvsimulator; do
  for name in RedCube CubeWith4Animations SkinnedLimb; do
