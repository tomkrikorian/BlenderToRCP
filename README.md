...
- Experimental, build-pinned Reality Composer Pro 3 `.import` generation for static multi-mesh and multi-material assets, single-mesh transform animation, and single- or multi-mesh skeletal assets that share one skeleton and animation contract. Baked base-color/opacity and roughness texture payloads are supported per material. Multi-material USD meshes are split into measured one-material RCP mesh resources without losing faces. The new multi-mesh skeletal lane is structurally validated but remains fail-closed and is not an RCP compatibility claim until its clean import, save/reopen, reimport, playback, and RealityKit runtime gates pass.
